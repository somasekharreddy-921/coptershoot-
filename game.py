...
warning = False
warning_counter = 0
warning_message = message_to_screen("!", font, 150, red)
...
def main_menu():
    ...
    while menu:
        ...
        # drawing background
        game_display.blit(sprites.background1, (0, 0))
        ...
        if godmode:
            ...
            title = message_to_screen("COPTER SHOOT", font, 50, black)
        controls_1 = message_to_screen(" ..............A GAME BY $OMUCHERRY.............", font, 30, black)
...
def game_loop():
    ...
    while not game_exit:
        ...
        while game_over:
            ...
            game_over_text = message_to_screen("GAME OVER", font, 50, black)
            your_score = message_to_screen("YOUR SCORE WAS: " + str(score), font, 50, black)
            if game_over_selected == "play again":
                play_again = message_to_screen("PLAY AGAIN", font, 45, white)
            else:
                play_again = message_to_screen("PLAY AGAIN", font, 45, black)
            if game_over_selected == "quit":
                game_quit = message_to_screen("QUIT", font, 45, white)
            else:
                game_quit = message_to_screen("QUIT", font, 45, black)
        ...
        # drawing boat
        game_display.blit(sprites.boat1, (boat1.x, boat1.y))
        ...
        if not player.wreck_start and not player.wrecked and not game_over:
            for draw_bullet in boat1.bullets:
                pygame.draw.rect(game_display, gray, (draw_bullet[0]+40, draw_bullet[1]+30, 20, 20))
            for move_bullet in range(len(boat1.bullets)):
                boat1.bullets[move_bullet][0] -= 10
                boat1.bullets[move_bullet][1] -= 10
            for del_bullet in boat1.bullets:
                if del_bullet[1] < -40:
                    boat1.bullets.remove(del_bullet)
        ...
        if score > 700 and boat_spawn_num == 100 and not boat1_alive:
            boat1.x = 800
            boat1_alive = True
        ...
        for hit_boat1 in bullets:
            if boat1.x < hit_boat1[0]+90 < boat1.x+110 or boat1.x < hit_spaceship[0]+100 < boat1.x+110:
                if boat1.y < hit_boat1[1]+40 < boat1.y+70 or boat1.y < hit_boat1[1]+50 < boat1.y+70:
                    if not boat1.x > 780:
                        pygame.mixer.Sound.play(explosion2)
                        bullets.remove(hit_boat1)
                        score += 200
                        boat1_alive = False
                        boat1.x = -110

        for hit_boat1 in bombs:
            if boat1.x < hit_boat1[0]+55 < boat1.x+110 or boat1.x < hit_spaceship[0]+75 < boat1.x+110:
                if boat1.y < hit_boat1[1]+70 < boat1.y+70 or boat1.y < hit_boat1[1]+90 < boat1.y+70:
                    if not boat1.x > 780:
                        pygame.mixer.Sound.play(explosion2)
                        bombs.remove(hit_boat1)
                        score += 200
                        boat1_alive = False
                        boat1.x = -110
        ...
        for hit_player in boat1.bullets:
            if player.x < hit_player[0] < player.x+100 or player.x < hit_player[0]+20 < player.x+100:
                if player.y < hit_player[1] < player.y+80 or player.y < hit_player[1]+20 < player.y+80:
                    pygame.mixer.Sound.play(explosion)
                    if not boat1.boat_hit_player:
                        player.damaged = True
                        player.health -= 1
                        boat1.bullets.remove(hit_player)

        # player-boat collision detection
        if boat1.x < player.x < boat1.x+110 or boat1.x < player.x+100 < boat1.x+110:
            if boat1.y < player.y < boat1.y+70 or boat1.y < player.y+80 < boat1.y+70:
                if not boat1.boat_hit_player:
                    pygame.mixer.Sound.play(explosion)
                    player.damaged = True
                    player.health -= 1
                    boat1.boat_hit_player = True
        ...
        # draw score
        game_display.blit(message_to_screen("SCORE: {0}".format(score), font, 30, black), (10, 10))

        # draw high score
        if score < highscore_int:
            hi_score_message = message_to_screen("HI-SCORE: {0}".format(highscore_int), font, 30, black)
        else:
            highscore_file = open('highscore.dat', "w")
            highscore_file.write(str(score))
            highscore_file.close()
            ...
            highscore_int = int(highscore_file.read())
            highscore_file.close()
            hi_score_message = message_to_screen("HI-SCORE: {0}".format(highscore_int), font, 30, yellow)
